...
#            box.col='red', text.col='red', cex=0.5)
  ...
  # ATR1 section:
  if (any(!is.na(data$ATF1) & !is.na(data$ATH2))) {
    ylb <- expression (paste ("ATF1  [", degree, "C]"))
    plot (DF <- data[, c("ATH2", "AT_A")], pch=20,ylab=ylb)
    lines (c(-70.,30.), c(-69.,31.), col="darkorange", lwd=2, lty=2)
    lines (c(-70.,30.), c(-71.,29.), col="darkorange", lwd=2, lty=2)
    par(new=T)
    plot (data$ATH2, data$ATF1 - data$ATH2, type='l', col='red',axes=F,xlab='',ylab='',ylim=c(-2,2))
    axis (4,col='red',col.axis='red')
    abline(h=c(-1,1),col='red',lty=2)
    mtext(expression (paste ("ATF1 - ATH2  [", degree, "C]")),4,3,col='red',cex=0.8)
    ...
    #                                "dashed lines: +/-1C error bands"), 
    #            box.col='red', text.col='red', cex=0.5)
    fm <- lm(ATF1~ATH2, data=data)
    coef <- coefficients (fm)
    if (coef[1] < 0.) {
      t <- sprintf ("ATF1=%.3f(ATH2)%.3f\n mean diff ATF1-ATH2=%.2f +/-%.2f", 
                    coef[2], coef[1], mean(data$ATF1-data$ATH2, na.rm=TRUE),
                    sd(data$ATF1-data$ATH2, na.rm=TRUE))
    } else {
      t <- sprintf ("ATF1=%.3f(ATH2)+%.3f\n mean diff ATF1-ATH2=%.2f +/-%.2f", 
                    coef[2], coef[1], mean(data$ATF1-data$ATH2, na.rm=TRUE), 
                    sd(data$ATF1-data$ATH2, na.rm=TRUE))
  ...
  if (any(!is.na(data$AT_A) & !is.na(data$ATH2))) {
    ...
    lines (c(-70.,30.), c(-71.,29.), col="darkorange", lwd=2, lty=2)
    par(new=T)
    plot (data$ATH2, data$AT_A - data$ATH2, type='l', col='red',axes=F,xlab='',ylab='',ylim=c(-2,2))
    axis (4,col='red',col.axis='red')
    abline(h=c(-1,1),col='red',lty=2)
    mtext(expression (paste ("AT_A - ATH2  [", degree, "C]")),4,3,col='red',cex=0.8)
#    DF$AT_A <- (data$ATH2 - data$AT_A)*5
#    points (DF, col='red', type='l', lwd=2)
#    lines (c(-70,30), c(5,5), col='red', lty=2)
##    lines (c(-70,30), c(-5,-5), col='red', lty=2)
