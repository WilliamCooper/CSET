...
RPlot5 <- function (data) { 
  ## needs DP_DPL, DP<DPR, DP_VXL, EW_DPR, EX_VXL, MR, ATX, 
...
#  title ("cyan line: equilibrium vapor pressure at ATX")
  ...
  data$RHDPL <- 100 * data$EW_DPL / MurphyKoop (data$ATX, data$PSXC)
  data$RHDPR <- 100 * data$EW_DPR / MurphyKoop (data$ATX, data$PSXC)
  plotWAC (data[, c("Time", "RHDPL", "RHDPR", "RHVXL")], lty=c(1,1,2), lwd=1, ylab="relative humidity [%]",cex.lab=1.5,cex.axis=1.5, legend.position='topright')
  # pulling legend out of plotWAC to increase font size
