...
RPlot22 <- function (data) {
  ...
  CUHSAS <- ncvar_get (netCDFfile, "CUHSAS_RWOOU")
  CPCASP <- ncvar_get (netCDFfile, "CS200_RWOOP")
  ...
  CellSizes <- ncatt_get (netCDFfile, "CUHSAS_RWOOU", "CellSizes")
  CellLimitsU <- CellSizes$value
  CellSizes <- ncatt_get (netCDFfile, "CS200_RWOOP", "CellSizes")
